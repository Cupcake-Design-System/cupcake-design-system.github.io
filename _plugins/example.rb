...
eos
      ...
      def example(output)
        "<div class=\"docs-example\"" + (@options[:id] ? " data-example-id=\"#{@options[:id]}\"" : "") + ">\n#{output}\n</div>"
      end


      def remove_example_classes(code)
        # Find `docs-` classes and remove them from the highlighted code. Because of how this regex works, it will also
        # remove classes that are after the `docs-` class. While this is a bug, I left it because it can be helpful too.
        # To fix the bug, replace `(?=")` with `(?=("|\ ))`.
        code = code.gsub(/(?!class=".)\ *?docs-.+?(?=")/, "")
